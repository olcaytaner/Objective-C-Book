...
- (NSMutableArray*) kelimeAra:(NSString*)arananKelime{
    ...
    for (SozlukKelime* kelime in _kelimeler){
...
- (void)parser:(NSXMLParser *)parser didStartElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI
 qualifiedName:(NSString *)qualifiedName attributes:(NSDictionary *)attributeDict {	
    if ([elementName isEqualToString:@"word"]) {
        ...
        kelime = [[SozlukKelime alloc] initWithAll:ad];
        NSString* sinif = [attributeDict objectForKey:@"lex_class"];
        if (sinif != NULL){
            kelime.sinif = sinif;
        }
        NSString* koken = [attributeDict objectForKey:@"origin"];
        if (koken != NULL){
            kelime.koken = koken;
        }
        deger = nil;
